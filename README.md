...
**--obito**: os casos de auto lesão cujo paciente foi a óbito não devem ser notificadas no SINAN NET de acordo com o instrutivo do VIVA. Nessa modalidade são identificadas palavras-chave no campo "observação" tais como "óbito", "morte" e "falecimento" na tentativa de identificar pacientes nas condições acima. Considerando que o campo "observação" não é codificado, as notificações que estão no arquivo de saída devem ser analisadas pelo usuário para que, na descrição do caso, se faça a constatação de que se trata mesmo de um óbito causado por lesão autoprovocada pelo paciente. Em caso afirmativo, a notificação precisa ser excluída.  
